...
class Sprint {
    ...
    *dates() {
        ...
        return yield* (function*() { let result = []; while (!date.isAfter(this.end)) {
            if (date.day() !== 0 && date.day() !== 6) { yield date.clone(); }
            result.push(date.add(1, 'days'));
        } return result; })();
    }
    datesCount() {
        let result = 0;
...
let initSprint = function(sprintConfig, date) {
