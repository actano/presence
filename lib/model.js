...
import {startOfWeek, toLocalDate, toInstant, toMoment} from './util'

export default function teamModel(team, currentDate) {
    let sprintStart = team.sprint.scrum ? team.sprint.start : null;
    let sprintEnd = team.sprint.scrum ? team.sprint.end : null;
    ...
    for (let member of team.members) {
        result.members.push(memberModel(member, toInstant(LocalDate.parse(result.range.start)), toInstant(LocalDate.parse(result.range.end))));
    }
...
function model(absence) {
    ...
    return {
        ...
        end: toMoment(absence.day.endDate()).diff(zero, 'minutes'),
