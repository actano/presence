...
function* absences(member, startDate, endDate) {
  for (const event of member.events) {
    for (const instance of event.instances(startDate, endDate)) {
      for (const day of instance.days(startDate)) {
        if (day.startDate().isAfter(endDate)) return
        const _absence = new Absence(member, instance.event, day)
        const date = toLocalDate(_absence.startDate()).toString()
        const absence = model(_absence)
        yield { date, absence }
      }
    }
  }
