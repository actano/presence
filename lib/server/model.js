...
const sprintSummary = (team) => {
  const sprintDays = team.sprint.datesCount()
  const sprintMembers = team.members.length
  const total = sprintDays * sprintMembers
  let avail = Number(total)

  for (const member of team.members) {
    for (const date of team.sprint.dates()) {
