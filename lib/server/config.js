...
const { npm_package_config_teams: teamsSource } = process.env
...
export default async function config(date) {
  debug('[begin] create config for %s', date)
  const isoDate = date.toString()

  const _config = yaml.safeLoad(fs.readFileSync(teamsSource))
  ...
  debug('[done] create config for %s', date)
  return _config
}
