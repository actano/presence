...
const parseTeamMember = (member) => {
  if (isString(member)) {
    ...
    return parseTeamMember({ name, remove })
  }
  const attendee = toArray(member.attendee)
  return { ...member, attendee }
}

const parseTeamMembers = (members) => {
  if (Array.isArray(members) && !members.length) return [{ remove: true }]
  return toArray(members).map(parseTeamMember)
}
...
const parseTeams = (teams) => {
  return toArray(teams).map(parseTeam)
}
    ...
    const teams = _teams
      .map((team) => {
        ...
        const members = filter(memberMap.values(), ({ remove }) => !remove)
          ...
          .map(member => ({ ...member, remove: undefined }))
