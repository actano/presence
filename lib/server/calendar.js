...
class Instance {
  constructor(event, date) {
    this.event = event
    this.startDate = date
    this.endDate = date.plusSeconds(event.duration())
  }
  ...
  * days(start) {
    ...
    yield* daysAfter(this.startDate, this.endDate)
  }
}
...
export function* instances(event) {
  if (!event.icalEvent.isRecurring()) {
    yield new Instance(event, icalTimetoInstant(event.icalEvent.startDate))
    return
  }

  for (const v of icalIterator(event.icalEvent)) {
    const date = icalTimetoInstant(v)
    yield new Instance(event, date)
  }
}
